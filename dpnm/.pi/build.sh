...
# docker buildx build --tag dpnm --file .pi/Dockerfile --platform linux/arm64 --cache-from dpnm:cache --cache-to dpnm:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar dpnm
...
sudo bash -c "echo dpnm > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/dpnm.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/dpnm.img
